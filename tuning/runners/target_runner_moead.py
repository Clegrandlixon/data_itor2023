...
def configure_experiment_hybrid(instance: str, allParameters: dict, config_id: int, run: int):
    ...
    #############################
    adaptedNeighbours = relevant_neighbours(allParameters['populationSize'], problem, allParameters["metric"])

    ls = ApplyManyOperators(problem, probability = allParameters['probabilityLocalSearch'], strategy= allParameters["lsStrategy"], neighbours= adaptedNeighbours)
    ...
    job = Job(
        algorithm = KnowledgeDiscoveryMOEAD(
            ...
            injection = PatternInjectionMOVRPTW(problem, probability = 0.0, diversificationFactor = injectionStrategy),
            localSearch= ls,
...
if __name__=='__main__':
    ...
    while cand_params:
        ...
        if param == "--popSize":
            ...
            allParams["lsStrategy"] = value
        elif param == "--metric":
            allParams["metric"] = value
        elif param == "--groups":
            if int(value) == 100:
