...
def read_arguments(argv):
    ...
    allParameters["populationSize"] = 40
    allParameters["sizeNeighborhood"] = 10
    allParameters["probabilityCrossover"] = 0.50
    allParameters["probabilityMutation"] = 0.00
    allParameters["probabilityLocalSearch"] = 0.25
    allParameters["nbGroups"] = 20
    allParameters["maxPatternSize"] = 5
    allParameters["patternsInjected"] = 100
    ...
    # Instance Parameters
    instanceParameters = ["nameAlgo", "seed", "benchmark", "idRun", "instanceNumber", "instanceType", "instanceSize"]
    # idRun // intermediateSubpath
    ...
    intParameters = ["seed","maxTime","populationSize","sizeNeighborhood","granularity","nbGroups","maxPatternSize","patternsInjected","extractionStrategy","injectionStrategy", "idRun"]
...
def read_front(path: str, nbObjectives: int = 2):
    ...
    with open(path) as file:
        ...
        for item in data:
            ...
            listObjectives = [float(x.strip()) for x in item[:-2].split("\t")][:nbObjectives]
...
def relevant_neighbours(nbSubproblems: int, problem, metric: str = "d3"):
    ...
    :param metric: The metric uses to compute the distances. It can be either "d1", "d2" or "d3". By default it should be "d2"
    """
    ...
    for l in range(nbSubproblems):
        ...
        for i in range(dimension):
            ...
            reducedNeighbours[l].append([j[1] for j in neighbours_i[1:problem.granularity]]) # discard i itself
...
def configure_experiment(allParameters: dict, subpath, path_front: str = None):
    ...
    problem_tag = os.path.join(subpath, "Run"+str(idRun), "Final")
    ...
    job = Job(
            algorithm = KnowledgeDiscoveryMOEAD(
                ...
                injection = PatternInjectionMOVRPTW(problem, allParameters["probabilityInjection"], diversificationFactor= allParameters["injectionStrategy"]), # probability = allParameters["probabilityInjection"]
                localSearch= ls,
                weight_files_path = "resources/MOEAD_weights",
                output_path = os.path.join("data", allParameters["nameAlgo"], subpath, "Run"+str(idRun), "Snapshots"),
                extrema_path= None,
                verbose= True,
                timeRecordData= 40, # nb of evaluations 
                termination_criterion = StoppingByTime(allParameters["maxTime"])),
            algorithm_tag= allParameters["nameAlgo"],
            problem_tag=problem_tag,
            run= idRun, # 1 by default when configurations are tested
            seed = initial_seed * idRun *  10
        )
...
def computePatterns(groupID: int, sizePattern: int, numberPatternsToDisplay, knowledge: StorePatterns):
    allPatterns = []
    patterns = knowledge.groups[groupID][sizePattern-2]
    listOfPatterns = [p for p in patterns]
    listOfPatterns.sort(key=lambda t: -t.frequency)
    patternsToDisplay = listOfPatterns[:numberPatternsToDisplay]
    for pattern in patternsToDisplay:
        allPatterns.append((pattern.pattern, pattern.frequency))
    return allPatterns
...
if __name__ == '__main__':
    ...
    if allParameters["benchmark"] == "Generated":
        if int(instanceNumber) < 10:
            instanceNumber = "0" + instanceNumber
            
    subpath = os.path.join(allParameters["benchmark"], allParameters["benchmark"]+"_"+instanceSize, instanceType + instanceNumber)
    ...
    if allParameters["displayKnowledge"]:
        knowledgeGenerated = job.algorithm.knowledge # member of the class StorePatterns
        maxPatternSize = allParameters["maxPatternSize"]
        ...
        group_slider = Slider(group_axis, "Group ID", 
                                0, numberOfGroups-1, valinit = init_group, valstep = 1)
        size_slider = Slider(patternSize_axis, "Size of Patterns", 
                                2, maxPatternSize, valinit = init_patternSize, valstep = 1)
        nbPattern_slider = Slider(nbPattern_axis, "Number of Patterns", 
                                1, 100, valinit = init_nbPatterns, valstep = 1)
