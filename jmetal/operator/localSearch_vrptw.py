...
class TwoOptStar(boVRPTWLocalSearch):
    ...
    def generate_neighborhood_candidate(self, candidate: tuple, solution: PermutationSolution):
        ...
        routes = solution.structure
        neighborhood = []
        customer_i =solution.structure[candidate[0]][candidate[1]]
        for i in range(len(routes)):
            if i != candidate[0]:
                current_route = routes[i]
                neighbours = []
                for j in range(1, len(current_route)-1):
                    customer_j = current_route[j]
                    if (not customer_j in solution.forbidden_elements) and (customer_j in self.neighbours[solution.associatedProblem][customer_i][:solution.attributes["granularity"]]):
    ...
    def compute_information_move(self, element1: tuple, element2: tuple, solution: PermutationSolution):
        ...
        new_route1, attributes_newR1 = self.problem.merge_Nsequences(list_sequences1, solution)
        new_route2, attributes_newR2 = self.problem.merge_Nsequences(list_sequences2, solution)
...
class Swap(boVRPTWLocalSearch):
    ...
    def generate_neighborhood_candidate(self, candidate: tuple, solution: PermutationSolution):
        ...
        for r in range(len(routes)):
            ...
            for j in range(1, len(current_route)-1):
                customer_j = current_route[j]
                if (r,j) != candidate and (not customer_j in solution.forbidden_elements) and (customer_j in self.neighbours[solution.associatedProblem][customer_i][:solution.attributes["granularity"]]):
...
class Relocate(boVRPTWLocalSearch):
    ...
    def generate_neighborhood_candidate(self, element: tuple, solution: PermutationSolution):
        ...
        routes = solution.structure
        neighborhood = []
        customer_i = solution.structure[element[0]][element[1]]
        for r in range(len(routes)):
            current_route = routes[r]
            neighbours = []
            for j in range(1, len(current_route)-1):
                customer_j = current_route[j]
                if (len(current_route) > 3) and (r,j) != element and (r,j+1) != element and (not customer_j in solution.forbidden_elements) and (customer_j in self.neighbours[solution.associatedProblem][customer_i][:solution.attributes["granularity"]]):
                    neighbours.append((r,j))
            neighborhood += neighbours
        random.shuffle(neighborhood)
        return neighborhood
...
class ApplyManyOperators(TwoOptStar, Swap, Relocate):
    ...
    def execute(self, solution: PermutationSolution) -> PermutationSolution:
        if random.random() > self.probability:
            solution.attributes["LS"] = False # LS not applied
            return solution
        ...
        solution.attributes["LS"] = True # LS applied
        return solution
