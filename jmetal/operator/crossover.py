...
class PMXCrossover(Crossover[PermutationSolution, PermutationSolution]):
    ...
    def execute(self, parents: List[PermutationSolution]) -> List[PermutationSolution]:
        ...
        offspring = [copy.copy(parents[0]), copy.copy(parents[1])] # don't use deepcopy
        permutation_length = offspring[0].number_of_variables

        rand = random.random()
        applied = rand <= self.probability
        if rand <= self.probability:
            ...
            offspring[0].needUpdate = True
            offspring[1].needUpdate = True
        return offspring, applied
