...
class MOVRPTW(PermutationProblem):
    ...
    def __init__(self, granularity: int, instance: str = None , referenceFront = None, idealPoint = None, nadirPoint = None):
        ...
        if not referenceFront is None:
            ...
            self.nadir = [max([i[0] for i in referenceFront])*1.2, max([i[1] for i in referenceFront])*1.2] # increase the nadir to see the convergence of HV (instead of having 0.0)
    ...
    def __read_from_file(self, filename: str):
        ...
        if benchmark == "Solomon" or benchmark == "Homberger":
    ...
    def attributes_sequence(self, t_sequence, solution: RoutingSolution):
        ...
        :param solution: a RoutingSolution with sequences up to date
    ...
    def merge_2sequences(self, sequence1, attributesSequence1, sequence2, attributesSequence2, solution, addDepot: bool):
        ...
        attributesRoute = self.concatenate_subsequences(indexesLeft, attributesSequence1, indexesRight, attributesSequence2)
    
        if addDepot:
            attributesRoute = self.add_depot_to_sequence((newRoute[0], newRoute[-1]), attributesRoute, solution)
    ...
    def merge_Nsequences(self, sequences, solution):
        ...
        if n == 1:
            ...
            attributesMerged = self.add_depot_to_sequence((mergedSequence[0], mergedSequence[-1]), attributesMerged, solution)
