...
class RoutingSolution(PermutationSolution):
    """ Class representing routing solutions """

    def __init__(self, number_of_variables: int, number_of_objectives: int, number_of_constraints: int = 0):
        super(PermutationSolution, self).__init__(number_of_variables, number_of_objectives, number_of_constraints)
        self.sequences = [[None for i in range (number_of_variables + 1)] for j in range(number_of_variables + 1)] 
        #self.reversed_sequences = [[None for i in range (number_of_variables + 1)] for j in range(number_of_variables + 1)]
        self.forbidden_elements = []
        self.structure = [] # only used if you need to store the solution in another format (like the tours for a vrptw)
        self.cycle = 0
        self.needUpdate = True
        self.associatedProblem = None

    def __copy__(self):
        new_solution = RoutingSolution(
            self.number_of_variables,
            self.number_of_objectives)
        new_solution.objectives = self.objectives[:].copy()
        new_solution.variables = self.variables[:].copy()
        new_solution.attributes = self.attributes.copy()
        new_solution.forbidden_elements = []
        new_solution.associatedProblem = self.associatedProblem
        new_solution.needUpdate = True
        return new_solution
