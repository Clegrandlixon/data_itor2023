...
from jmetal.util.solution import print_function_values_to_file, print_variables_to_file, read_solutions, print_structures_to_file
...
class Job:
    ...
    def __init__(self, algorithm: Algorithm, algorithm_tag: str, problem_tag: str, run: int, seed: int):
        ...
        self.seed = seed
        self.results = []
        self.finalMetrics = {'HYPERVOLUME': 0, 'EPSILON': 0, 'mGD': 0, 'mIGD': 0}

    def execute(self, output_path: str = ''):
        self.algorithm.run()
        self.results, additionalSolutions = self.algorithm.get_result() # 
        
        observables = self.algorithm.get_observable_data()
        computedMetrics = self.algorithm.metrics
        
        if observables['HYPERVOLUME'] != 'NA':
            computedMetrics['Iteration'].append(self.algorithm.iterations)
            computedMetrics['Time'].append(round(observables['COMPUTING_TIME'], 2))
            computedMetrics['SizeFront'].append(len(self.results))
            computedMetrics['Hypervolume'].append(round(observables['HYPERVOLUME'], 0))
            computedMetrics['unaryHypervolume'].append(round(observables['unaryHYPERVOLUME'], 5))
            computedMetrics['Epsilon'].append(int(observables['EPSILON']))
            computedMetrics['GD'].append(round(observables['GD'], 2))
            computedMetrics['IGD'].append(round(observables['IGD'], 2))

            self.finalMetrics['HYPERVOLUME'] = round(observables['HYPERVOLUME'], 0)
            self.finalMetrics['unaryHYPERVOLUME'] = round(observables['unaryHYPERVOLUME'], 5)
            self.finalMetrics['EPSILON'] = int(observables['EPSILON'])
            self.finalMetrics['mGD'] = round(observables['GD']/len(self.results), 2)
            self.finalMetrics['mIGD'] = round(observables['IGD']/len(self.results), 2)

        if output_path:
            
            
            file_name = os.path.join(output_path, 'allFUN.tsv')
            print_function_values_to_file(additionalSolutions, filename=file_name)

            file_name = os.path.join(output_path, 'allVAR.tsv')
            print_variables_to_file(additionalSolutions, filename=file_name)

            file_name = os.path.join(output_path, 'allSTR.tsv')
            print_structures_to_file(additionalSolutions, filename=file_name)
            

            file_name = os.path.join(output_path, 'ndFUN.tsv')
            print_function_values_to_file(self.results, filename=file_name)            

            file_name = os.path.join(output_path, 'ndVAR.tsv')
            print_variables_to_file(self.results, filename=file_name)

            file_name = os.path.join(output_path, 'ndSTR.tsv')
            print_structures_to_file(self.results, filename=file_name)



            file_name = os.path.join(output_path, 'METRICS.tsv')

            with open(file_name, 'w') as of:
                of.write("Iter.\tTime\tSize\tHypervolume\tunaryHV\tEps.\tGD\tIGD\n") # header
                nbLines = len(computedMetrics['Iteration'])
                for i in range(nbLines):
                    of.write(str(computedMetrics['Iteration'][i]) + '\t')
                    of.write(str(computedMetrics['Time'][i]) + '\t')
                    of.write(str(computedMetrics['SizeFront'][i]) + '\t')
                    of.write(str(computedMetrics['Hypervolume'][i]) + '\t')
                    of.write(str(computedMetrics['unaryHypervolume'][i]) + '\t')
                    of.write(str(computedMetrics['Epsilon'][i]) + '\t')
                    of.write(str(computedMetrics['GD'][i]) + '\t')
                    of.write(str(computedMetrics['IGD'][i]) + '\n')

            # The file STATS contain the detail of the execution of the algorithm
            file_name = os.path.join(output_path, 'STATS.tsv')
            with open(file_name, 'w+') as of:
                of.write('Computing time: ' + str(observables['COMPUTING_TIME']) + '\n \n')
                of.write('Stats per bloc: ' + '\n' + '\n')
                of.write('Extraction: ' + '\n')
                of.write(' - Time: ' + str(self.algorithm.times["Extraction"]) + '\n')
                of.write(' - Number Applied: ' + str(self.algorithm.countAll['Extraction']) + '\n \n')
                of.write('Injection: ' + '\n')
                of.write(' - Time: ' + str(self.algorithm.times["Injection"]) + '\n')
                of.write(' - When Crossover applied before: \n')
                of.write('  * Number Applied: ' + str(self.algorithm.countAll['Injection']['crossover']) + '\n')
                of.write('  * Number Improved: ' + str(self.algorithm.countImproved['Injection']['crossover']) + '\n')
                of.write('  * Mean Improvement: ' + str(round(self.algorithm.meanImprovement['Injection']['crossover']/max(1, self.algorithm.countAll['Injection']['crossover']), 3)) + '\n')
                of.write(' - When nothing applied before: \n')
                of.write('  * Number Applied: ' + str(self.algorithm.countAll['Injection']['nothing']) + '\n')
                of.write('  * Number Improved: ' + str(self.algorithm.countImproved['Injection']['nothing']) + '\n')
                of.write('  * Mean Improvement: ' + str(round(self.algorithm.meanImprovement['Injection']['nothing']/max(1, self.algorithm.countAll['Injection']['nothing']), 3)) + '\n \n')
                of.write('Evaluation: ' + '\n')
                of.write(' - Time: ' + str(self.algorithm.times["Evaluation"]) + '\n')
                of.write(' - Number Applied: ' + str(self.algorithm.evaluations) + '\n \n')
                of.write('Archive Update: ' + '\n')
                of.write(' - Time: ' + str(self.algorithm.times["Archive"]) + '\n \n')
                of.write('Local Search: ' + '\n')
                of.write(' - Time: ' + str(self.algorithm.times["LocalSearch"]) + '\n')
                of.write(' - When Crossover and Injection applied before: \n')
                of.write('  * Number Applied: ' + str(self.algorithm.countAll['LocalSearch']['both']) + '\n')
                of.write('  * Number Improved: ' + str(self.algorithm.countImproved['LocalSearch']['both']) + '\n')
                of.write('  * Mean Improvement: ' + str(round(self.algorithm.meanImprovement['LocalSearch']['both']/max(1, self.algorithm.countAll['LocalSearch']['both']), 3)) + '\n')
                of.write(' - When Injection applied before: \n')
                of.write('  * Number Applied: ' + str(self.algorithm.countAll['LocalSearch']['injection']) + '\n')
                of.write('  * Number Improved: ' + str(self.algorithm.countImproved['LocalSearch']['injection']) + '\n')
                of.write('  * Mean Improvement: ' + str(round(self.algorithm.meanImprovement['LocalSearch']['injection']/max(1, self.algorithm.countAll['LocalSearch']['injection']), 3)) + '\n')
                of.write(' - When Crossover applied before: \n')
                of.write('  * Number Applied: ' + str(self.algorithm.countAll['LocalSearch']['crossover']) + '\n')
                of.write('  * Number Improved: ' + str(self.algorithm.countImproved['LocalSearch']['crossover']) + '\n')
                of.write('  * Mean Improvement: ' + str(round(self.algorithm.meanImprovement['LocalSearch']['crossover']/max(1, self.algorithm.countAll['LocalSearch']['crossover']), 3)) + '\n')
                of.write(' - When nothing applied before: \n')
                of.write('  * Number Applied: ' + str(self.algorithm.countAll['LocalSearch']['nothing']) + '\n')
                of.write('  * Number Improved: ' + str(self.algorithm.countImproved['LocalSearch']['nothing']) + '\n')
                of.write('  * Mean Improvement: ' + str(round(self.algorithm.meanImprovement['LocalSearch']['nothing']/max(1, self.algorithm.countAll['LocalSearch']['nothing']), 3)) + '\n \n') 
                of.write('\n')
